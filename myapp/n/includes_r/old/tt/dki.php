...
$list = explode(",",$listx);
$table = $_GET['table'];
if($table == 'bigbluebuttonbn_logs')
	{
		$out = $db_moodle->select('mdl_'.$table, $list, array("ORDER" => ["courseid"=>"ASC", "id"=>"ASC" ]));
	}
			else
				{
					$out = $db_moodle->select('mdl_'.$table, $list);
				}
